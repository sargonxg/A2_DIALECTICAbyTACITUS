...
export async function POST(request: Request) {
  ...
  const blocks = pipelineBlockCatalog.filter((block) =>
    ["source-upload", "aletheia-ontology-profile", "temporal-episode-splitter", "primitive-extraction", "neo4j-memory-write", "abstract-knowledge-graph", "agent-result-terminal", "benchmark-evaluation"].includes(block.id),
  );
