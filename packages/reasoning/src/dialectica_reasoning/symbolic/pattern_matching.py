...
class PatternMatcher:
    ...
    async def detect_spoiler_dynamics(self, workspace_id: str) -> list[PatternMatch]:
        ...
        edges = await self._gc.get_edges(workspace_id)
        events = await self._gc.get_nodes(workspace_id, label="Event")
        await self._gc.get_nodes(workspace_id, label="Outcome")
        matches: list[PatternMatch] = []
            ...
            if len(evidence) >= 2:
                ...
                matches.append(
                    PatternMatch(
                        ...
                        description=f"Actor '{actor_id}' shows spoiler behaviour — opposing resolution.",  # noqa: E501
                        intervention_recommendation=(
                            "Include potential spoilers in the process, address their core interests, "  # noqa: E501
    ...
    async def detect_hurting_stalemate(self, workspace_id: str) -> list[PatternMatch]:
        ...
        for c in conflicts:
            ...
            costly_events = [ev for ev in events if getattr(ev, "severity", 0) >= 0.6]
            ...
            matches.append(
                PatternMatch(
                    ...
                    description=(
                        ...
                        f"{len(actor_alliances)} actors, with {len(opposed_edges)} opposition edges."  # noqa: E501
                    ),
                    intervention_recommendation=(
